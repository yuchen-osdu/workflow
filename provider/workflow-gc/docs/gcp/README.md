...
| `OSDU_AIRFLOW_VERSION`             | `airflow3` OR `airflow2`                      | Allows configuring the Airflow engine version used by Workflow service, by default `airflow3` is used                                                | no         | -                                                            |
| `COMPOSER_CLIENT`                  | `IAM` OR `IAP`                                | Allows configuring authentication method used by Workflow to authenticate its requests to Airflow, by default `IAM` (Google Cloud IAM / ADC) is used | no         | -                                                            |
