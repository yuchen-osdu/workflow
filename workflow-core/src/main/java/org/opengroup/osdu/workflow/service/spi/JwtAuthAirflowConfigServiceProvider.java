...
/**
 * Builds the {@link AirflowConfig} (url/username/password) for the {@code JwtAuth} client type used
 * by the Airflow 3 {@code api/v2} backend. The credentials are exchanged for a JWT at runtime by
 * {@link org.opengroup.osdu.workflow.service.JwtAuthAirflowApiClient}.
 */
@Component
@Slf4j
@RequiredArgsConstructor
public class JwtAuthAirflowConfigServiceProvider implements IAirflowConfigServiceProvider {
  ...
  public AirflowConfig create(Map<String, Object> configMap) {
    log.info("Creating Airflow config for JwtAuth");
    ...
    if (!httpAirflowAllowed && !url.toLowerCase().startsWith("https://")) {
      throw new AppException(
          HttpStatus.SC_BAD_REQUEST, "Invalid URL", "External Airflow URL must use HTTPS");
    }
