...
@Slf4j
public class ExternalAirflowConfigFactory {

  private final List<IAirflowConfigServiceProvider> providers;

  public AirflowConfig createExternalAirflowConfig(
      String airflowApiClientType, Map<String, Object> configMap) {

    log.info("Creating External Airflow Config for type: {}", airflowApiClientType);
    return providers.stream()
        .filter(provider -> provider.supports(airflowApiClientType))
        .findFirst()
        .map(provider -> provider.create(configMap))
        .orElseThrow(
            () -> {
